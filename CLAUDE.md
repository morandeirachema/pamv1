...
- **`store`** — `Store` interface + domain types (`Target`, `Credential`, `AuditEvent`). Two implementations: `memstore` (tests/demo) and `pgstore` (Postgres via pgx, embedded `schema.sql` applied idempotently at startup). Sentinel errors `ErrNotFound`/`ErrConflict` map to HTTP/SSH errors upstream.
