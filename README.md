...
- **Active Directory & Entra ID login** — sign in with an AD username + password over **LDAPS**, or with **Microsoft Entra ID (Azure AD)**; directory groups / app roles map to the four roles (highest privilege wins) and `POST /api/login` issues a short-lived session token that works in the portal and the proxy. Both sources can be enabled together; local tokens and break-glass remain as the emergency path.
...
- **Windows targets (WinRM)** — run commands on Windows hosts through `POST /api/targets/{id}/winrm`; the credential is injected just-in-time (AD-joined domain accounts work), the transcript is recorded with a SHA-256 in the audit, and the caller never sees the secret. RDP brokering is on the roadmap.
