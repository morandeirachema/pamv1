...
- [~] **Approval + resume + short-lived single-use tokens**: *shipped* — the `require_approval` effect parks a call, an approver decides via `GET /v1/approvals` + `POST /v1/approvals/{id}/decision`, execute-on-approve injects JIT (the human decision satisfies the target four-eyes gate), and the agent collects the result once with a single-use `broker_tokens` JTI (`POST /v1/tool-calls/{id}/resume`); per-agent rate limits + argument-size caps. *Next in this increment*: more tools — `rotate_credential`, `reveal_credential` (default-deny), `list_targets`/`list_credentials`, `ssh_exec`
