# pamv1 for System Administrators — how it works
...
flowchart LR
  ...
  ADM -->|manage| API
  OP -->|"session (their pam key)"| SSHP
  OP -->|"psql (their pam key)"| DBP
  API --- VAULT
  SSHP --- VAULT
  DBP --- VAULT
...
```bash
ssh -p 2222 alice_token@web-01@pam.example      # SSH; password = alice's pam token
